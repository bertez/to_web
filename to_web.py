...
class BatchProcess(object):
    """This class processes a list of videos to video formats usable by modern
    ...
    encodings = {
        'mp4': ['ffmpeg -y -i {0} -pass 1 -vcodec libx264 -vpre \
                fast_firstpass -b {3}k -bt {4}k -threads 4 {2}-f rawvideo \
                -an -y /dev/null', 'ffmpeg -y -i {0} -pass 2 -acodec libfaac \
                -ab {4}k -ac 2 -vcodec libx264 -vpre slow -b {3}k -bt {3}k \
                -threads 4 {2}{1}'],
    ...
    def __init__(self, arguments, output_dir, width, height, disable_mp4,
                 disable_ogv, videobr, audiobr):
        ...
        self.errorfile = self.outputdir + os.sep + \
            str(time.time()) + '_errors_job.log'
    ...
    def process(self):
        ...
        for video in self.filelist:
            ...
            for finalformat, command in self.encodings.items():
                ...
                final_file = self.outputdir + os.sep + \
                    video_output + '.' + finalformat
                for cmd in command:

                    wh_string = "" if self.width is None or \
                        self.height is None else self.wh_patterns[
                            finalformat].format(self.width, self.height) + ' '

                    subp = cmd.format(
                        video, final_file, wh_string, self.videobr,
                        self.audiobr)
                    ...
                    try:
                        ...
                        self.errors[video].append(
                            'System error. Check dependencies.')
                    except ValueError:
    ...
    def __repr__(self):
        resize_info = 'The videos will not be resized' if self.width is None \
            or self.height is None else 'The output video(s) will be this \
                size: {0}x{1}'.format(
    ...
    You'll find the compressed videos here: {4}
        ...
        """.format(str(self.filelist), resize_info, self.videobr,
                   self.audiobr, self.outputdir)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Example usage: to_web.py --videos video1.avi video2.avi \
        --width 960 --height 540 --output-dir upload")

    group = parser.add_argument_group('resize')

    parser.add_argument(
        '--videos', action='store', dest='videos', default=[], required=True,
        help='Video to process. You can add multiple.', nargs='+'
    )

    parser.add_argument(
        '--output-dir', action='store', dest='output_dir',
        default='videos_to_web', help='Output directory. \
        Path. Default: videos_to_web')
    ...
    parser.add_argument(
        '--video-bitrate', action='store', dest='videobr', default=1500,
        help='Output video bitrate. Number. Default: 1500', type=int)

    parser.add_argument(
        '--audio-bitrate', action='store', dest='audiobr', default=128,
    ...
    parser.add_argument(
        '--version', action='version', version='%(prog)s 0.1 by @bertez')

    results = parser.parse_args()

    # sys.exit(results)

    my_batch = BatchProcess(
        results.videos, results.output_dir, results.width, results.height,
        results.disable_mp4, results.disable_ogv, results.videobr,
        results.audiobr)
